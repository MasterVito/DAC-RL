...
if __name__ == '__main__':
    ...
    parser.add_argument("--system_prompt_path", default='', type=str)
    ...
    parser.add_argument("--log_probs", default=None, type=int, help="Set to >0 to enable log_probs output.")
    ...
    sample_params = SamplingParams(temperature=args.temperature, top_p=args.top_p, top_k=args.top_k, n=args.n_sampling, max_tokens=args.max_tokens, stop=["<end>", "\] \] \]", "\n\n\n"], logprobs=args.log_probs)

    dataset = load_dataset("json", data_files=args.input_path)["train"]
    assert not (args.log_probs and args.temperature > 0), "Activate log_probs when temperature > 0 is meaningless, as the outputs are sampled and the probabilities of the sampled tokens maybe not align with the recorded log_probs."


    if len(args.system_prompt_path) > 0:
        system_path = open(args.system_prompt_path, "r").read()
        replace_token = r"{REPLACE}" if r"{REPLACE}" in system_path else r"{Original}"
        dataset = dataset.map(lambda x: {args.prompt_key: system_path.replace(replace_token, x[args.prompt_key])}, num_proc=64)

    tokenizer = AutoTokenizer.from_pretrained(args.model_name_or_path)
    add_generation_prompt = False if "llama" in args.model_name_or_path.lower() else True
    dataset = dataset.map(lambda x: {"vllm_input": tokenizer.apply_chat_template([{"content": x[args.prompt_key], 'role': 'user'}], add_generation_prompt=add_generation_prompt, tokenize=False)}, num_proc=64)
    ...
    outputs_text = [[_.outputs[i].text.split("<|endoftext|>")[0] for i in range(len(_.outputs))] for _ in outputs]
    
    print(f">>> Finishing inference")
    print(f"Dataset Keys: {dataset.column_names}")
    ### Check prediction in the dataset
    if "prediction" in dataset.column_names:
        dataset = dataset.remove_columns(["prediction"])
    
    dataset = dataset.add_column("prediction", outputs_text)
