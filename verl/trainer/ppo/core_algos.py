...
def compute_gpg_outcome_advantage(
    ...
    **kwargs,
    ...
    """
    Compute advantage for GPG, operating only on Obatch_for_sft = model_inputs["for_sft"] if "for_sft" in model_inputs.keys() else Noneutcome reward
    (with only one scalar reward for each response).
    Args:
        token_level_rewards: `(torch.Tensor)`
            shape: (bs, response_length)
...
def compute_policy_loss(
    ...
    #     mask = (covs > args["cov_lb"]) & (covs < args["cov_ub"])
    #     valid_covs = covs[mask]
    #     clip_idx = random_select(valid_covs, num=select_num)

    #     # detach selected samples
...
# @register_policy_loss("rollout_learning")
# def compute_policy_loss_rollout_learning(
#     old_log_prob: torch.Tensor,
#     log_prob: torch.Tensor,
#     advantages: torch.Tensor,
#     response_mask: torch.Tensor,
...
#     config: Optional[DictConfig | AlgoConfig] = None,
#     rollout_log_probs: torch.Tensor | None = None,
#     batch_for_sft: np.ndarray | None = None,
# ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
#     """
#     Compute the clipped policy objective and related metrics for PPO.
